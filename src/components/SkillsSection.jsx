...
function SkillBar({ skill, index }) {
  ...
  return (
    <div className={`space-y-2 group anim-fade-rise anim-fade-rise-d${Math.min(index + 1, 5)}`}>
      {/* Label row */}
      ...
      <div className="relative h-6 bg-gray-950 border border-gray-800 overflow-hidden group-hover:border-gray-700 transition-colors">
        {/* Filled portion */}
        <div
          className="h-full anim-bar-fill relative overflow-hidden"
          style={{ "--bar-width": `${skill.level}%`, background: `linear-gradient(90deg, ${skill.color}99, ${skill.color})`, boxShadow: `0 0 10px ${skill.color}60` }}
        >
          {/* Stripe overlay */}
          ...
          <span className="absolute right-2 top-1/2 -translate-y-1/2 pixel-text text-white font-bold drop-shadow-lg" style={{ fontSize: "0.45rem" }}>
            LVL {lvl}
          </span>
        </div>
...
export default function SkillsSection() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-12 anim-fade-rise">
      {/* Title */}
        ...
        <div className="mt-10 pt-8 border-t border-gray-900">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              ...
              <div
                key={label}
                className={`text-center border border-${color}-900/50 bg-${color}-950/20 py-4`}
